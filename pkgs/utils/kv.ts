...
export class BunSqliteKeyValue {
  ...
  constructor(filename?: string, options?: Options) {
    ...
    this.db.run(
      "CREATE TABLE IF NOT EXISTS items (key TEXT PRIMARY KEY, value BLOB, expires INT)"
    );
    this.db.run(
    ...
    this.deleteExpiredStatement = this.db.query(
      "DELETE FROM items WHERE expires < $now"
    );
    ...
    this.getAllItemsStatement = this.db.query(
      "SELECT key, value, expires FROM items"
    );
    this.getItemStatement = this.db.query(
      "SELECT value, expires FROM items WHERE key = $key"
    );
    ...
    this.getKeyStatement = this.db.query(
  ...
  getItems<T = any>(
    startsWithOrKeys?: string | string[]
  ): Item<T>[] | undefined {
    let records: Record[];
  ...
  getItemsObject<T = any>(
    startsWithOrKeys?: string | string[]
  ): { [key: string]: T | undefined } | undefined {
    const items = this.getItems(startsWithOrKeys);
    if (!items) return;
    return Object.fromEntries(
      items.map((item) => [item.key, item.value as T | undefined])
    );
  }
